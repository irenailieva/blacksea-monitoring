...
# Pomorie Area (Lake, Town, and Bay)
...
def run():
    ...
    jp2_paths = {
        'b2': base_dir / "T35TNH_20240610T085559_B02_10m.jp2",
        'b3': base_dir / "T35TNH_20240610T085559_B03_10m.jp2",
        'b4': base_dir / "T35TNH_20240610T085559_B04_10m.jp2",
        'b8': base_dir / "T35TNH_20240610T085559_B08_10m.jp2",
        'scl': base_dir / "T35TNH_20240610T085559_SCL_20m.jp2"
    }

    cropped_paths = {}
    print("Checking cropped bands for Pomorie...")
    for key, path in jp2_paths.items():
        if not path.exists():
            print(f"Error: Missing {path}")
            return
        
        cropped_path = base_dir / f"cropped_pomorie_{key}.tif"
        if not cropped_path.exists():
            crop_band(str(path), str(cropped_path), POMORIE_ROI_WGS84, is_scl=(key=='scl'))
    ...
    CORRECT_OFFSET = False
    output_uncorrected = base_dir / "pomorie_WITHOUT_correction.tif"
    process_scene(cropped_paths, str(output_uncorrected), model)
    
    # Run WITH correction
    print("\n--- Running WITH correction ---")
    CORRECT_OFFSET = True
    output_corrected = base_dir / "pomorie_WITH_correction.tif"
